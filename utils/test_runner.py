...
def process_task(test_cases, root_url):
    results = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:  # 每个进程使用 4 个线程
        futures = [executor.submit(run_test_case, case, root_url) for case in test_cases]
